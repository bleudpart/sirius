...
- **Mises à jour automatiques** : l'application installée vérifie les nouvelles versions sur
  ...
  **Publier une nouvelle version :**
  1. Augmente `version` dans `frontend/package.json` (ex. `1.0.1`)
  2. Compile : `BUILD.bat` (ou `yarn build && yarn backend:build` si le HUD/backend ont changé)
  3. Publie : `set GH_TOKEN=ton_token_github` puis `yarn release` depuis `frontend/`
