...
def agora_api():
    session = requests.Session()
    session.headers.update({"Content-Type": "application/json"})
    response = session.post(f"{BASE_URL}/api/auth/local-session", timeout=30)
...
class TestAgoraRegression:
    def test_objectif_get_put_and_persistence(self, agora_api):
        initial = agora_api.get(f"{BASE_URL}/api/agora/objectif", timeout=30)
        assert initial.status_code == 200, initial.text
        initial_data = initial.json()
        for field in ("montant", "gagne_mois", "progression_pct", "mois"):
            assert field in initial_data

        updated = agora_api.put(f"{BASE_URL}/api/agora/objectif", json={"montant": 5000}, timeout=30)
        assert updated.status_code == 200, updated.text
        assert updated.json().get("montant") == 5000.0

        persisted = agora_api.get(f"{BASE_URL}/api/agora/objectif", timeout=30)
        assert persisted.status_code == 200, persisted.text
        assert persisted.json().get("montant") == 5000.0
        assert isinstance(persisted.json().get("progression_pct"), (int, float))

    def test_coach_history_contract(self, agora_api):
        response = agora_api.get(f"{BASE_URL}/api/agora/coach/history", timeout=30)
        assert response.status_code == 200, response.text
        sessions = response.json().get("sessions")
        assert isinstance(sessions, list)
        assert all(isinstance(item, dict) and "_id" not in item for item in sessions)

    def test_relance_without_email_returns_clear_400_and_cleanup(self, agora_api):
        deal_id = None
        name = f"TEST_NoEmail_{uuid.uuid4().hex[:10]}"
        try:
            created = agora_api.post(
            ...
            if deal_id:
                deleted = agora_api.delete(f"{BASE_URL}/api/agora/deals/{deal_id}", timeout=30)
                assert deleted.status_code in (200, 404), deleted.text

        listing = agora_api.get(f"{BASE_URL}/api/agora/deals", timeout=30)
