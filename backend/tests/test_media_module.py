...
def test_media_proxy_resolves_official_youtube_embed():
    ...
    assert descriptor.embed_url == "https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ"
...
def test_media_routes_archive_and_oracle(media_client):
    resolved = media_client.post(
        "/media/resolve",
        json={"provider": "tiktok", "url": "https://www.tiktok.com/@sirius/video/7123456789012345678"},
    )
    assert resolved.status_code == 200
    assert resolved.json()["state"]["embed_url"].endswith("7123456789012345678")
    ...
    assert oracle.json()["provider"] == "tiktok"
...
def test_media_websocket_acknowledges_control(media_client):
    ...
    with media_client.websocket_connect("/media/ws") as websocket:
        ...
        assert initial["state"]["provider"] == "deezer"
